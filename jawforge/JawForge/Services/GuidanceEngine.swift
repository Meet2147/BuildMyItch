    ...
    static func recommendations(for m: JawlineMetrics) -> [Recommendation] {
        ...
        if m.gonialAngleScore < 75 {
            recs.append(Recommendation(
                id: "angle",
                title: String(localized: "Sharpen your jaw angle"),
                detail: String(localized: "Your jaw corner measures \(Int(m.gonialAngle))°, on the softer side. The masseter muscle sits right on that corner — training it adds visible width and hardness there. Chewing training and resistance presses are your highest-leverage work."),
        ...
        if m.widthRatioScore < 75 {
            if m.jawToFaceWidthRatio < 0.80 {
                recs.append(Recommendation(
                    id: "width",
                    title: String(localized: "Build jaw width"),
                    detail: String(localized: "Your jaw sits narrow relative to your upper face. Progressive chewing work grows the masseters, which sit exactly where width is measured — this is the most trainable metric of all."),
                    exerciseIDs: ["chewing", "jaw_resistance"]
                ))
            } else {
                recs.append(Recommendation(
                    id: "width",
                    title: String(localized: "Refine rather than build"),
                    detail: String(localized: "Your jaw is already wide relative to your upper face, so skip heavy chewing work — focus on definition through posture and under-chin tightening instead."),
        ...
        if m.lowerFaceScore < 75 {
            recs.append(Recommendation(
                id: "proportion",
                title: String(localized: "Rebalance the lower face"),
                detail: String(localized: "Your lower-face proportion sits outside the classical balance point. Consistent tongue posture (mewing) and chin tucks improve how the lower third carries — and posture changes photograph faster than you'd expect."),
        ...
        if m.symmetryScore < 78 {
            recs.append(Recommendation(
                id: "symmetry",
                title: String(localized: "Even out left/right balance"),
                detail: String(localized: "Your jaw sits slightly off your facial midline. The usual culprits are one-sided chewing, sleeping on one side, and leaning on one hand. Alternate chewing sides deliberately and add controlled side glides."),
        ...
        recs.append(Recommendation(
            id: "foundation",
            title: String(localized: "The two things that matter most"),
            detail: String(localized: "1) Body-fat percentage: no exercise outshines a lower body-fat level — the jawline you're building is revealed, not created, by leanness. 2) Posture: forward-head posture visually erases up to half your jawline. Chin tucks and all-day mewing are non-negotiable foundations."),
